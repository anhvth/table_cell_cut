...
def main():
    ...
    if a.mode == "export":
        ...
        h, w= inputs_shape[0], inputs_shape[1]
        input_resized = tf.image.resize_images(\
                        tf.expand_dims(inputs, 0), (CROP_SIZE, CROP_SIZE), tf.image.ResizeMethod.NEAREST_NEIGHBOR)[0]
        batch_input_tensor = tf.expand_dims(tf.identity(input_resized / 255, 'batch_input_tensor'), axis=0)
        with tf.variable_scope("generator"):
            batch_output = deprocess(
                create_generator(preprocess(batch_input_tensor), 3)[0])
            batch_output = tf.image.resize_images(batch_output, (h, w), method=tf.image.ResizeMethod.AREA)
        outputs = tf.identity(tf.cast(batch_output[0]*255, tf.uint8), name='outputs')
