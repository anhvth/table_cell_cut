...
parser.add_argument('--mode', default='train', help='directory to outputs')
...
if args.mode == 'train':
    ...
    path_inp = glob(os.path.join(args.input_dir1,'*'))
    ...
    if args.mode == 'train':
        for i, (p_in, p_out) in enumerate(zip(path_inp, path_out)):
            ...
            for _ in range(50):
                ...
                img_combined = cv2.resize(img_combined, (0, 0), fx=.25, fy=.25)
                cv2.imwrite(os.path.join(args.output_dir,'{}_{}.png'.format(name, _)), cv2.cvtColor(img_combined, cv2.COLOR_BGR2RGB))
    else:
        for i, p_in in enumerate(path_inp):
            name = os.path.split(p_in)[-1]
            img_inp1 = read_image(p_in, 'input')
            img_inp2 = read_image(p_in, 'output1')
            img_inp3 = read_image(p_in, 'output2')
            img_inp = np.stack([img_inp1,img_inp2,img_inp3], axis=-1)
            img_inp = cv2.resize(img_inp, (0, 0), fx=.25, fy=.25)
            cv2.imwrite(os.path.join(args.output_dir,'{}'.format(name)), cv2.cvtColor(img_inp, cv2.COLOR_BGR2RGB))