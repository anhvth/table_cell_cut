...
parser.add_argument("--mode", default="train", choices=["train", "test", "export", "export_2nd_model"])
...
def main():
    ...
    if a.mode == "export_2nd_model":
        # export the generator to a meta graph that can be imported later for standalone generation

        inputs = tf.placeholder(tf.float32, [None, None, None, 3], 'inputs')
        input_padding, top, left, bot, right = pad(inputs)
        with tf.variable_scope("generator"):
            g_outputs = create_generator(preprocess(inputs), 3)
            outputs, features = g_outputs[0], g_outputs[-1]
